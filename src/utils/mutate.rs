...
fn mutate_nucleotide(nucleotide: u8, rng: &mut ThreadRng) -> u8 {
    ...
    let nuc_choices = Uniform::new_inclusive(0, 3).unwrap();
...
pub fn mutate_fasta(file_struct: &HashMap<String, Vec<u8>>) -> HashMap<String, Vec<u8>> {
    ...
    for (name, sequence) in file_struct {
        ...
        println!("Sequence {} is {} bp long", name, sequence_length);
        ...
        println!("Adding {} mutations", num_positions);
